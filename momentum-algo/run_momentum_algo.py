...
# Configrations
# define start date and end date for the analysis
...
prices_file_name = 'df_prices_clean.csv'#'df_prices_clean.csv'#'df_prices_clean.csv'#'leading_indices_prices_clean.csv' #df_prices_clean.csv

# Data loading is done outside of this main function. We assume to have a csv with stock prices per day
...
own_capital = 1#1 # how much of our own capital we are using in decimal (1 = 100%). etoro asked to use zero and make long-short portfolio balanced
...
if __name__ == '__main__':
    ...
    # taking only the first 500 columns (companies)
    #df_returns = df_returns.iloc[:, :100].copy()
    # number of stocks possible to trade with
    n_stocks = len(df_returns.columns)
    print(f'Number of stocks in the dataset: {n_stocks}')
    ...
    # taking only 1 year (just for debugging purposes, otherwise we would take all the data))
    #df_returns = df_returns.iloc[-12*20:, ]  # taking only last XX years of data, i.e. 24 months

    df_all_strategies = calc_momentum_strategies(df_returns, lookback, holding_period, n_long, n_short, short_cap, long_cap)
    ...
    summary_file_name = f'momentum_own_capital_{own_capital}_start_date_{start_date}.csv'
    ...
    figure_saving_file_name = f'momentum_NAVs_own_capital_{own_capital}_start_date{start_date}.png'
    plot_nav_values(df_NAVs, results_path, saving_file_name=figure_saving_file_name)
